...
import { DownloadIcon, BookOpen, X, ExternalLink, Loader2 } from "lucide-react";
import { useDocuments } from "@/lib/hooks/public/useDocumentPublicHooks";
import { downloadDocument } from "@/lib/api/functions/public/useDocumentPublicApi"; // ← import
...
function PdfViewerModal({
  ...
  const handleDownload = async () => {
    setDownloading(true);
    try {
      await downloadDocument(doc.id, doc.file_name); // uses axios directly
    } finally {
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b border-gray-200 bg-gray-50 shrink-0">
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-8 h-8 rounded-lg bg-[#068847]/10 flex items-center justify-center shrink-0">
              <BookOpen size={16} className="text-[#068847]" />
            </div>
...
const MagazineCard = ({
  ...
  // ✅ Now uses the authenticated API endpoint instead of raw blob fetch
        ...
        {(isLoading || featured) && (
          <div className="font-dm-sans mx-auto mb-16 max-w-7xl">
            {isLoading ? (
              ...
              <MagazineCard doc={featured} featured onRead={setViewingDoc} />
            )}
          </div>
        )}
