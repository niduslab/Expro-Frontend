...
import { FieldLabel, FieldError, Divider, inputClass, textareaClass, STATUS_OPTIONS } from "./blogPostModalShared";
...
export function PostDetailsSection({ formData, errors, onTitleChange, set }: PostDetailsSectionProps) {
  return (
    <div className="flex flex-col relative top-[24px] gap-[16px]">
      ...
      <div className="flex gap-2 w-full">
        <div className="w-1/2">
          ...
          <input className={inputClass} placeholder="Post title" value={formData.title}
            onChange={(e) => onTitleChange(e.target.value)} />
          <FieldError message={errors.title} />
        </div>
        <div className="w-1/2">
          <FieldLabel label="Title (Bangla)" />
          <input className={inputClass} placeholder="পোস্টের শিরোনাম"
      ...
      <div>
        ...
        <input className={inputClass} placeholder="post-slug" value={formData.slug}
      ...
      <div>
        ...
        <textarea className={textareaClass} rows={2} placeholder="Short summary..."
      ...
      <div>
        ...
        <textarea className={textareaClass} rows={6} placeholder="Write your post content here..."
          value={formData.content} onChange={(e) => set("content", e.target.value)} />
        <FieldError message={errors.content} />
      </div>

      <div>
        <FieldLabel label="Featured Image URL" />
        <input className={inputClass} placeholder="https://example.com/image.jpg"
...
export function PublishingSettingsSection({ formData, categories, set }: PublishingSettingsSectionProps) {
  return (
    <div className="flex flex-col relative top-[48px] gap-[16px]">
      ...
      <div className="flex gap-2 w-full">
        <div className="w-1/2">
          ...
          <select className={inputClass} value={formData.category_id ?? ""}
        ...
        <div className="w-1/2">
          ...
          <select className={inputClass} value={String(formData.status)}
...
export function TagsSection({ tags, tagInput, onTagInputChange, onAddTag, onRemoveTag }: TagsSectionProps) {
  return (
    <div className="flex flex-col relative top-[72px] gap-[16px]">
      <p className="font-semibold text-[18px] leading-[150%] tracking-[-0.01em] text-[#030712]">Tags</p>

      <div>
        <FieldLabel label="Add Tags" />
        <div className="flex gap-2">
          <input className={inputClass} placeholder="Type a tag and press Add"
            ...
            {tags.map((tag) => (
              <span key={tag}
                ...
                <button type="button" onClick={() => onRemoveTag(tag)}
                  className="text-[#068847] hover:text-red-500 font-bold ml-0.5">×</button>
              </span>
            ))}
          </div>
        )}
